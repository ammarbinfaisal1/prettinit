...
const prettierrcDefaults = require("./prettierrc-defaults.json");
...
module.exports = {
	 ...
	 */
	fix(settings) {
		const { qoutes, indentation, trailingComma, arrowParens, tabWidth, printWidth } = settings;
		...
		settings.printWidth = Number(printWidth);

		// Remove defaults
		for (const key of Object.keys(settings)) {
			if (settings[key] === prettierrcDefaults[key]) delete settings[key];
		}
